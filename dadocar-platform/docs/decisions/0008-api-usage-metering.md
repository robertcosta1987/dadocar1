...
2. **`api_products`** — the sellable consult types per API (`api`, `code`, `name`, `unit_price_brl`). Seeded with the CheckTudo products at the Placas360 storefront prices (66 Veículo Total R$44,90; 67 Essencial R$29,90; 13 Decod+Precificador R$19,90; 71 Cadastrais R$9,90; 76 Decod+FIPE R$17,90; 241 Decod V.4 R$12,90; 65 Consulta 65 R$60,00).
...
6. **Admin "Usage Report for APIs"** (`/admin/uso-apis`, admin-only). Aggregates the ledger **API → subscription → user → product** (a subscription can have multiple users), split live vs cache, with amounts to charge and first/last timestamps, plus a per-subscription **Export to PDF** (print-friendly document for the browser's Save-as-PDF). `usageReport({ subscriptionId })` already accepts a per-customer scope so the same view can later be shown to a paying customer for their own usage.
7. **APIM products — one per consult type.** Published products in API Management — `checktudo-veiculo-total` (66), `-65` (65, R$60), `-essencial` (67), `-decod-precificador` (13), `-dados-cadastrais` (71), `-decod-fipe` (76), `-decod-v4` (241) — each with a Moneycar subscription (`moneycar-checktudo-*`). One product per type because prices differ. These are the billing/subscription **catalog**; CheckTudo traffic is **not yet routed through APIM**, and even once it is, the live-vs-cache split stays in `api_usage` (APIM can't see the app cache).

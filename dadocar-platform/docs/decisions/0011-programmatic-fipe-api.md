...
1. **Endpoint**: `GET /api/v1/fipe/plate/{placa}` (Node runtime, `maxDuration=60`). Returns `{ ok, placa, source: live|cache, consultaId, fipe: { marca, modelo, anoModelo, anoFabricacao, chassi, numMotor, combustivel, codigoFipe, valorAtual, historico[] } }`.

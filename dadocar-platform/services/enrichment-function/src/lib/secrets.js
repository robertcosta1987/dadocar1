...
// system-assigned Managed Identity (which Bicep already granted
// `Key Vault Secrets User` on the dev vault).
//
// Locally, the same credential chain falls through to the developer's
// `az login` session — so as long as the developer has Secrets Get rights
// they can run the Function locally with no env-var changes.
//
// We cache values in-process for 5 min. That's plenty for the bearer-token
// flow (Infocar tokens live 8h) and small enough that rotating a secret in
// Key Vault recovers quickly without a Function restart.
// ─────────────────────────────────────────────────────────────────────────────
"use strict";
